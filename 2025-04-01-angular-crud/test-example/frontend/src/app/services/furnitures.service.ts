...
export class FurnituresService {
  ...
  async getAll(): Promise<Furniture[]> {
    const observable = this.httpClient.get<Furniture[]>(`${environment.restServerUrl}/furnitures`)
    const list = firstValueFrom(observable)
    return list
  }

  async create(draft: Draft): Promise<Furniture[]> {
    const observable = this.httpClient.post<Furniture[]>(`${environment.restServerUrl}/furnitures`, draft)
